...
import { type Progress, type Action, type RegionId, isVillageUnlocked, isForestUnlocked, isRegionUnlocked } from '../world/progression';
import {Vegetation,Paths,Village,Forest,Atmosphere,Wildlife,Mountains} from './Landscape';
import {move} from '../world/navigation';
const entryStart:Record<RegionId,{x:number;z:number}>={hill:spawn,village:{x:1,z:-22},forest:{x:-22,z:-3}};
...
function CameraRig({progress,dispatch,controls,onPosition}:Props) {
 ...
 const next=move(pos.current,controls.yaw,forward,side,d,speed,{village:isVillageUnlocked(progress),forest:isForestUnlocked(progress)});pos.current.x=next.x;pos.current.z=next.z;
...
const enterableIds:RegionId[]=['hill','village','forest'];
...
function Scene(props:Props){return <>
 ...
 <Terrain/><Paths/><Vegetation/><Village unlocked={isVillageUnlocked(props.progress)}/><Forest unlocked={isForestUnlocked(props.progress)}/><Atmosphere/><Mountains/><Wildlife/>
