...
import { EffectComposer, Bloom, GodRays, HueSaturation, BrightnessContrast, Vignette, Noise } from '@react-three/postprocessing';
import { BlendFunction } from 'postprocessing';
import * as THREE from 'three';
import { heightAt, fbm, regions, shrines, spawn, clareira, bosque, regionAt, type RegionKey } from '../world/data';
import { REGION_ATMOSPHERE, MAP_FOG, SHRINE_COLORS } from '../world/palette';
...
function Terrain() {
 ...
 const dBosque=Math.hypot(x-bosque.x,z-bosque.z);if(dBosque<20)c.lerp(deep,Math.max(0,1-dBosque/20)*.4);
 colors.push(c.r,c.g,c.b);}g.setAttribute('color',new THREE.Float32BufferAttribute(colors,3));g.computeVertexNormals();return g;},[]);
...
function Scene(props:SceneProps){const atmo=props.progress.mode==='map'?null:REGION_ATMOSPHERE[props.region];
 const fogColor=atmo?atmo.fog:MAP_FOG.color,fogNear=atmo?atmo.fogNear:MAP_FOG.near,fogFar=atmo?atmo.fogFar:MAP_FOG.far;
 return <>
 <color attach="background" args={['#172d30']}/><fog attach="fog" args={[fogColor,fogNear,fogFar]}/>
 <ambientLight intensity={.55} color={atmo?atmo.ambient:'#ffffff'}/><hemisphereLight args={[atmo?atmo.hemiSky:'#bac8c0',atmo?atmo.hemiGround:'#242c25',1.1]}/>
 <directionalLight position={[-30,32,-20]} color={atmo?atmo.sun:'#ffcc86'} intensity={atmo?atmo.sunIntensity:3} castShadow shadow-mapSize={[2048,2048]} shadow-camera-left={-75} shadow-camera-right={75} shadow-camera-top={75} shadow-camera-bottom={-75} shadow-camera-near={1} shadow-camera-far={160} shadow-bias={-.0015} shadow-normalBias={.025}/>
 <mesh ref={props.sunRef} visible={props.progress.mode==='explore'&&props.region==='forest'} position={[bosque.x-18,38,bosque.z-14]}><sphereGeometry args={[2.4,16,16]}/><meshBasicMaterial color="#d8f5e0" toneMapped={false}/></mesh>
 ...
 {shrines.map(s=>{const sc=SHRINE_COLORS[s.id];const lit=props.progress.attuned.includes(s.id);return <group key={s.id} position={[s.x,heightAt(s.x,s.z),s.z]}>
 <mesh position={[0,1,0]}><boxGeometry args={[.65,2,.5]}/><meshStandardMaterial color="#7e8170"/></mesh>
...
export default function World(props:Props){
 const [region,setRegion]=useState<RegionKey>('hill');
 const sunRef=useRef<THREE.Mesh>(null!);
 const handlePosition=(x:number,z:number)=>{props.onPosition(x,z);const r=regionAt(x,z);setRegion(prev=>prev===r?prev:r);};
 useEffect(()=>{if(props.progress.mode==='map')setRegion('hill');},[props.progress.mode]);
 const godRays=region==='forest'&&props.progress.mode==='explore';
 return <Canvas shadows camera={{position:[0,91,42],fov:49,near:.1,far:220}} dpr={[1,1.75]} gl={{antialias:true,powerPreference:'high-performance',toneMapping:THREE.ACESFilmicToneMapping}}>
 <Scene {...props} onPosition={handlePosition} region={region} sunRef={sunRef}/>
 <EffectComposer multisampling={0}>
 <Bloom mipmapBlur intensity={.65} luminanceThreshold={.82} luminanceSmoothing={.15} radius={.6}/>
 {godRays?<GodRays sun={sunRef} blendFunction={BlendFunction.SCREEN} samples={40} density={.85} decay={.92} weight={.4} exposure={.35} clampMax={1} kernelSize={2}/>:<></>}
 <HueSaturation saturation={-.08}/>
 <BrightnessContrast brightness={-.02} contrast={.08}/>
 <Vignette eskil={false} offset={.32} darkness={.55}/>
 <Noise opacity={.035} premultiply blendFunction={BlendFunction.OVERLAY}/>
 </EffectComposer>
 </Canvas>;}
