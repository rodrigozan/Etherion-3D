...
export const regions = [
  ...
  { id: 'temple_clearing', name: 'Clareira do Templo', subtitle: 'Região desconhecida', x: -38, z: -16 },
...
export function heightAt(x: number, z: number) {
  return 1.5 + 5 * Math.exp(-(x*x/380 + (z-13)**2/650)) + 1.4*Math.sin(x*.12)*Math.cos(z*.1) + .6*Math.sin(x*.3+z*.13) + 7*Math.exp(-((x+28)**2/110+(z+35)**2/170)) + 6*Math.exp(-((x-30)**2/130+(z+24)**2/140)) + 3*Math.exp(-((x+29)**2/260+(z+3)**2/220)) + 2*fbm(x,z);
}
